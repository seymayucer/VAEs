...
class VAE(nn.Module):
    ...
    def __init__(self, label, image_size, channel_num, kernel_num, z_size):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(self.channel_num, self.kernel_num//4,kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(self.kernel_num//4),
            nn.ReLU(),
            nn.Conv2d(self.kernel_num//4, self.kernel_num//2,kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(self.kernel_num//2),
            nn.ReLU(),
            nn.Conv2d(self.kernel_num//2, self.kernel_num,kernel_size=4, stride=2, padding=1),
        ...
        # projection
        self.project = nn.Linear(z_size,self.feature_volume, z_size)

        # decoder
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(self.kernel_num, self.kernel_num // 2,kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(kernel_num//2),
            nn.ReLU(),
            nn.ConvTranspose2d(self.kernel_num // 2, self.kernel_num // 4,kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(kernel_num // 4),
            nn.ReLU(),
            nn.ConvTranspose2d(self.kernel_num // 4, self.channel_num,kernel_size=4, stride=2, padding=1),
            nn.Sigmoid()
        )
